...
export async function findDirectInstruments(universe,eligibilityReader,env,fetcher,blockedSymbols=[],asOfDay=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Madrid'}).format(new Date())){
 const targetByRegion={"United States":20,Europe:20,"Asia-Pacific":20};const foundByRegion={"United States":0,Europe:0,"Asia-Pacific":0};const eligible=[];const complete=()=>Object.entries(targetByRegion).every(([region,target])=>foundByRegion[region]>=target);
 const groups={"United States":[],Europe:[],"Asia-Pacific":[]};for(const instrument of universe)groups[regionForSymbol(instrument.symbol)].push(instrument);
 const dayNumber=Math.floor(Date.parse(asOfDay+'T00:00:00Z')/86400000);for(const items of Object.values(groups)){if(!items.length)continue;const offset=((dayNumber*17)%items.length+items.length)%items.length;const rotated=items.slice(offset).concat(items.slice(0,offset));items.splice(0,items.length,...rotated);}
 const sampled=[];const maxLength=Math.max(...Object.values(groups).map(items=>items.length));for(let index=0;index<maxLength;index++)for(const region of Object.keys(groups))if(groups[region][index])sampled.push(groups[region][index]);
 for(let index=0;index<sampled.length&&!complete();index+=100){
  let batch;
  try{batch=await eligibilityReader(env,{mode:'real',instrumentIds:sampled.slice(index,index+100).map(item=>item.instrumentId)},fetcher);}
  catch(error){
   if(!(error instanceof ConnectionError)||error.status!==429)throw error;
   await pause(5000);
   batch=await eligibilityReader(env,{mode:'real',instrumentIds:sampled.slice(index,index+100).map(item=>item.instrumentId)},fetcher);
  }
  ...
  if(index+100<sampled.length&&!complete())await pause(400);
 }
 return eligible;
}
