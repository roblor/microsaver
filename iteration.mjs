...
export class Iteration {
 ...
 async init(){await mkdir(this.directory,{recursive:true});try{this.data=JSON.parse(await readFile(join(this.directory,'state.json'),'utf8'));this.data.settings=settings(this.data.settings);this.data.chatUsage=this.data.chatUsage||{};this.data.yellowOrders=this.data.yellowOrders||[];this.data.trades=this.data.trades||[];for(const trade of this.data.trades){if(trade.status==='open')trade.status='submitted';trade.providerState??='unknown';trade.providerFeedback??='No eToro response receipt was saved for this earlier request.';}if(!Array.isArray(this.data.reviews)||!this.data.usage||!Array.isArray(this.data.audit))throw Error();}catch(e){if(e.code!=='ENOENT')throw new Error('Local state cannot be read. Restore state.json before continuing.');}return this;}
 ...
 try{
 const portfolio=await this.refresh();const universe=await this.universeReader(this.env,this.fetcher);const eligibility=[];for(let index=0;index<universe.length;index+=100)eligibility.push(...await this.eligibilityReader(this.env,{mode:'real',instrumentIds:universe.slice(index,index+100).map(item=>item.instrumentId)},this.fetcher));const eligibleById=new Map(eligibility.filter(item=>item.allowOpenPosition&&item.minOrderAmount!==null&&item.minOrderAmount<300&&!item.settlementTypes.includes('realFutures')&&!/\.FUT$/i.test(item.symbol||'')).map(item=>[String(item.instrumentId),item]));const affordableUniverse=universe.filter(item=>eligibleById.has(String(item.instrumentId)));if(!affordableUniverse.length)throw new ConnectionError('eToro returned no currently openable, non-futures instruments with a minimum below 300 USD. No review created.');
