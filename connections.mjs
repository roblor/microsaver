...
export async function readInstrumentEligibilities(env=process.env,{mode='real',instrumentIds}={},fetcher=fetch){
 ...
 return (raw?.eligibilities||[]).map(eligibility=>{const minimums=[eligibility.minPositionExposure,...(eligibility.leverageConfigs||[]).map(config=>config?.minPositionAmount)].filter(value=>typeof value==='number'&&Number.isFinite(value)&&value>0);return {instrumentId:String(eligibility.instrumentId),symbol:typeof eligibility.symbol==='string'?eligibility.symbol:null,allowOpenPosition:eligibility.allowOpenPosition===true,minOrderAmount:minimums.length?Math.max(...minimums):null,settlementTypes:[...new Set((eligibility.leverageConfigs||[]).map(config=>config?.settlementType).filter(value=>typeof value==='string'))]};});
}
