...
export async function readInstrumentEligibilities(env=process.env,{mode='real',instrumentIds}={},fetcher=fetch){
 if(!env.ETORO_API_KEY||!env.ETORO_USER_KEY)throw new ConnectionError('eToro keys are missing. Run Start-Microsaver.ps1 to load them.',503);
 if(!['real','demo'].includes(mode)||!Array.isArray(instrumentIds)||instrumentIds.length<1||instrumentIds.length>100||instrumentIds.some(id=>(typeof id!=='string'&&typeof id!=='number')||!/^\d{1,20}$/.test(String(id))))throw new ConnectionError('Invalid eToro instrument list.',400);
 const path=mode==='demo'?'/api/v2/trading/info/demo/eligibility':'/api/v2/trading/info/eligibility';
 const raw=await postJson('https://public-api.etoro.com'+path,{'x-api-key':env.ETORO_API_KEY,'x-user-key':env.ETORO_USER_KEY,'x-request-id':crypto.randomUUID()},{instrumentIds:instrumentIds.map(Number),currency:'USD'},fetcher);
 return (raw?.eligibilities||[]).map(eligibility=>{const configs=eligibility.leverageConfigs||[];const settlementTypes=[...new Set(configs.map(config=>config?.settlementType).filter(value=>typeof value==='string'))];const realMinimums=configs.filter(config=>config?.settlementType==='real').map(config=>config?.minPositionAmount).filter(value=>typeof value==='number'&&Number.isFinite(value)&&value>0);return {instrumentId:String(eligibility.instrumentId),symbol:typeof eligibility.symbol==='string'?eligibility.symbol:null,allowOpenPosition:eligibility.allowOpenPosition===true,minOrderAmount:realMinimums.length?Math.max(...realMinimums):null,settlementTypes,canOpenDirect:eligibility.allowOpenPosition===true&&realMinimums.length>0};});
}
export async function readInstrumentEligibility(env=process.env,{mode='real',instrumentId}={},fetcher=fetch){const found=await readInstrumentEligibilities(env,{mode,instrumentIds:[instrumentId]},fetcher);const eligibility=found.find(item=>String(item.instrumentId)===String(instrumentId));if(!eligibility)throw new ConnectionError('eToro did not return trading eligibility for this instrument.',502);return eligibility;}
...
export async function submitOrder(env=process.env,order,fetcher=fetch){
 if(!env.ETORO_API_KEY||!env.ETORO_USER_KEY)throw new ConnectionError('eToro keys are missing. Run Start-Microsaver.ps1 to load them.',503);
 if(!order||!['demo','real'].includes(order.mode)||!['buy','sell'].includes(order.transaction)||!['open'].includes(order.action)||order.settlementType!=='real')throw new ConnectionError('Invalid order request.',400);
 ...
 try{response=await fetcher('https://public-api.etoro.com'+path,{method:'POST',headers,redirect:'error',signal:AbortSignal.timeout(20000),body:JSON.stringify({action:'open',transaction:order.transaction,instrumentId:order.instrumentId,settlementType:'real',orderType:'mkt',leverage:1,amount:order.amount,orderCurrency:'usd'})});}
